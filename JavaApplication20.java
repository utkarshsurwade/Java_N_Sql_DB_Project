...
if (a.getSource()==b1)
{
  ...
  stat = con.prepareStatement("insert into data values (?, ?, ?, ?, ?,?,?,?)");
                ...
                stat.setInt(6,Integer.parseInt(phone));
    ...
    ResultSet rs = null;  //USED FOR STORING THE RECORDS WHICH COMES FROM TABLE
      ...
      try {  
         ...
         while (rs.next()) { 
           ...
           String foo = String.format("%-20s%-20s%-20s%-20s%-20s%-20s%-20s%-20s %s", rs.getString(1) , rs.getString(2) , rs.getString(3) , rs.getString(4) , rs.getString(5) , rs.getString(6) , rs.getString(7) , rs.getString(8) , rs.getString(9));
           System.out.println(foo);
         }  
      }
...
class searchdb extends Frame implements ActionListener{
    ...
    add(b2);
    b2.setBounds(25,500,100,50);
    add(t2);
    t2.setBounds(130,510,50,25);
    //add(ll1);
    //ll1.setBounds(190,510,165,25);
    //ll1.setBackground(Color.red);
    add(b3);
    b3.setBounds(25,600,80,50);
